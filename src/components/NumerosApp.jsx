...
const faqs = [
  {
    ...
    a: 'В течение 2–5 минут после оплаты. Наш сервис автоматически генерирует разбор и отправляет его на указанный email. Если письмо не пришло — проверьте папку «Спам». В редких случаях задержка до 10 минут.',
  },
  {
    q: 'Это шаблонный текст или персональный разбор?',
    a: 'Полностью персональный. Каждый разбор генерируется индивидуально на основе вашей конкретной даты рождения с помощью GPT-4. Два человека с разными датами получат абсолютно разные тексты — разные числа, разные характеристики, разные рекомендации.',
  },
  {
  ...
  {
    q: 'Подходит ли разбор для мужчин?',
    a: 'Да. При заказе вы указываете пол, и разбор составляется с учётом этого: для мужчин акцент на действии, карьере и достижениях; для женщин — на интуиции, отношениях и самореализации. Метод одинаково точен для обоих полов.',
  },
  {
    q: 'Можно ли вернуть деньги?',
    a: 'Поскольку разбор генерируется персонально под вас сразу после оплаты, возврат возможен только если PDF не был доставлен по технической причине. Напишите нам — разберём ситуацию и поможем.',
  },
  {
